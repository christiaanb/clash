...
testReducer = all (\(a,b) -> a == toInteger b) $ zip referenceOutput reducerOutput
  where
    reducerOutput   = map fst $ catMaybes $ simulate reducer siminput
    referenceOutput = [foldl1 (+) 
                        (map (\z -> toInteger (fst z)) 
                          (filter (\x -> (snd x) == i) siminput)) | i <- [0..(snd $ last siminput)]]
